...
def main():
    ...
    forward_button['command'] = lambda: forward_callback(mqtt_client, left_speed_entry.get(), right_speed_entry.get())
    root.bind('<Up>', lambda event: forward_callback(mqtt_client, left_speed_entry.get(), right_speed_entry.get()))

    left_button = ttk.Button(main_frame, text="Left")
    left_button.grid(row=3, column=0)
    # left_button and '<Left>' key
    left_button['command'] = lambda: left_callback(mqtt_client, left_speed_entry.get(), right_speed_entry.get())
    root.bind('<Left>', lambda event: left_callback(mqtt_client, left_speed_entry.get(), right_speed_entry.get()))
    ...
    right_button['command'] = lambda: right_callback(mqtt_client, left_speed_entry.get(), right_speed_entry.get())
    root.bind('<Right>', lambda event: right_callback(mqtt_client, left_speed_entry.get(), right_speed_entry.get()))

    back_button = ttk.Button(main_frame, text="Back")
    back_button.grid(row=4, column=1)
    # back_button and '<Down>' key
    back_button['command'] = lambda: back_callback(mqtt_client, left_speed_entry.get(), right_speed_entry.get())
    root.bind('<Down>', lambda event: back_callback(mqtt_client, left_speed_entry.get(), right_speed_entry.get()))
...
def forward_callback(mqtt_client, left_speed_entry, right_speed_entry):
    mqtt_client.send_message("drive", [left_speed_entry, right_speed_entry])
