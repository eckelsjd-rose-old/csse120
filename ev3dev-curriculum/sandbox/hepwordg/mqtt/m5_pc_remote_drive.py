...
Authors: David Fisher and Derek Hepworth.
...
def main():
    ...
    stop_button['command'] = lambda: stop_callback(mqtt_client, left_speed_entry, right_speed_entry)
    root.bind('<Space>', lambda event: stop_callback(mqtt_client, left_speed_entry, right_speed_entry))
...
def left_callback(mqtt_client, left_speed, right_speed):
    mqtt_client.send_message("drive", [-int(left_speed), right_speed])
...
def right_callback(mqtt_client, left_speed, right_speed):
    mqtt_client.send_message("drive", [left_speed, -int(right_speed)])


def back_callback(mqtt_client, left_speed, right_speed):
    mqtt_client.send_message("drive", [-int(left_speed), -int(right_speed)])
