...
Much like we did in eckelsjd of the motors unit, later we will show you different (better) ways to use buttons.
...
def main():
    ...
    while True:
        ...
        if btn.down:
            while True:
                ev3.Leds.set_color(ev3.Leds.LEFT, led_colors[current_color_index
                                                             % len(led_colors)])
                ev3.Leds.set_color(ev3.Leds.RIGHT, led_colors[
                    current_color_index % len(led_colors)])
                time.sleep(.01)
        ...
        if btn.backspace:
            break
    time.sleep(0.01)  # Best practice to have a short delay to avoid
    # working too hard between loop iterations.
