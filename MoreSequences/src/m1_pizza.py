...
def run_test_draw_points_on_circle():
    ...
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  50 purple dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    draw_points_on_circle(window, circle, 50, 'purple')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  Many slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 12, 'red', 7)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        line = rg.Line(seq[k % len(seq)], seq[(k+1) % len(seq)])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  20 blue lines on orange circle, ' \
            'hops = 25.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'orange'
    fancy_polygon(window, circle, 20, 25, 'blue', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(number_of_lines):
        line = rg.Line(seq[k % len(seq)], seq[(k + hops_to_next_point) % len(
            seq)])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
