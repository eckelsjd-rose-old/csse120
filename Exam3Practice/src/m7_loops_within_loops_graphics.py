...
def many_hourglasses(window, square, m, colors):
    ...
    for k in range(m):
        ...
        hourglass(window, k + 1, rect.get_center(), radius, color)

        c1.x = c1.x + (2 * (k + 1)) * radius
        c2.x = c2.x + (2 * (k + 2)) * radius

        c1.y = c1.y - math.sqrt(3 * (radius ** 2))
        c2.y += math.sqrt(3 * (radius ** 2))
    ...
    #    TIME ESTIMATE:  ALL f*ing day (warning: this problem is
        # challenging)
    # ------------------------------------------------------------------
