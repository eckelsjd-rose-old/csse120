...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        corner_h_1 = rg.Point(intersection_center.x - (width / 2),
                              intersection_center.y - (letter_thickness / 2))
        corner_h_2 = rg.Point(intersection_center.x + (width / 2),
                              intersection_center.y + (letter_thickness / 2))

        corner_v_1 = rg.Point(intersection_center.x - (letter_thickness / 2),
                              intersection_center.y - (letter_thickness / 2))
        corner_v_2 = rg.Point(intersection_center.x + (letter_thickness /
                                                       2), corner_v_1.y +
                              height)

        self.h_rect = rg.Rectangle(corner_h_1, corner_h_2)
        self.v_rect = rg.Rectangle(corner_v_1, corner_v_2)
    ...
    def clone(self):
        ...
        h1 = self.h_rect.clone()
        v1 = self.v_rect.clone()
        center = h1.get_center()
        h1.fill_color = self.h_rect.fill_color
        h1.outline_color = self.h_rect.outline_color

        corner_left_h1 = h1.get_upper_left_corner()
        corner_right_h1 = h1.get_lower_right_corner()

        width = corner_right_h1.x - corner_left_h1.x
        letter_thickness = corner_right_h1.y - corner_left_h1.y

        corner_left_v1 = v1.get_upper_left_corner()
        corner_right_v1 = v1.get_lower_right_corner()

        height = corner_right_v1.y - corner_left_v1.y

        tt = CapitalT(center, width, height, letter_thickness)
        tt.set_colors(h1.fill_color, h1.outline_color)
